...
## NEXT UP — Goals section (untouched, this is the focus)
The Goals tab is still a placeholder (`client/src/pages/GoalsPage.tsx` → `<Placeholder>`). Backend is mostly ready; the page is the build.
- **Locked spec** (`project_task_manager.md`): goal hierarchy **Annual > Half-year > Monthly > Weekly per Life Area**; daily tasks optionally linked to weekly goals; lightweight progress on Today/Week; auto-prompted weekly/monthly/half-year reviews (reconciliation already exists separately).
- **Already built:** `Goal` model (areaId, text, icon, pct, color, **period** weekly|monthly|half_year|annual) + full CRUD `/api/goals`. `Task.goalId` exists. Client has `GoalsSidebar` (Today's "this week's goals" + today-by-area) and `AddGoalModal` (`components/today/AddGoalModal.tsx`). `today/useToday` already loads `goals` into state and has `addGoal`.
- **What's missing / to design:** the Goals *page* itself — show the four-level hierarchy grouped by life area, parent→child links (a weekly goal rolls up to a monthly → half-year → annual), progress derived from linked tasks (not just manual `pct`), create/edit/delete at each level, and the link between a weekly goal and the daily tasks under it. The current `Goal` model has `period` but **no parent link** — adding goal nesting (a `parentId`) is likely the first model change. Decide: derive `pct` from child goals / linked task completion vs keep manual. There is no `goals.html` mockup; design fresh, matching the cockpit aesthetic (frosted glass, Nunito, area colors).
- **Repo seam:** mirror the Today pattern. Goals page should read via the same repo seam so demo mode works (`localRepo` seed has 3 goals; extend the seed with a small hierarchy for demo). Real path uses `/api/goals`.
