...
- **Phase 4 (part 3a) done — Google Calendar OAuth plumbing (inert).** Read-only OAuth (auth-code + refresh) via plain fetch. Server: `GoogleAccount` + `BudgetExclusion` models, `services/google.ts`, `routes/google.ts` (`/status`, `/auth-url`, `/callback` with signed-state, account list/patch/delete, `/events`, `/exclude` POST+DELETE). Features stay off (503 / `configured:false`) until `GOOGLE_CLIENT_ID/SECRET` set in `server/.env` (see `.env.example`). Client: `google/api.ts` + `GoogleAccountsSection` in Settings (connect / per-source color+toggle / disconnect; inert + guest states).
  - **BLOCKED on creds:** Akshu is creating the Google Cloud OAuth client (guided). Once `server/.env` has the client id/secret, next step = wire the events-as-fixed-blocks display in the cockpit + the per-event/recurring-series "don't deduct" mute toggle (server `/events` already flags `deduct`, `/exclude` mutes a `seriesKey`). Redirect URI to register: `http://localhost:4000/api/google/callback`; JS origin `http://localhost:5173`.
- **Next: Phase 4 (part 3b)** — finish GCal (events in cockpit + mute UI) once creds land. Plus streak logic (hardcoded `streakDays={12}`), and unbuilt Phase 3 leftovers: recurring tasks + manual fixed blocks.
